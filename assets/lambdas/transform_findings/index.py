...
class TransformFindings:
    ...
    def __process_record(self, object_key):
        ...
        for item in raw_list:
            logger.info(f'raw_list={item}')
            if item['source'] == 'aws.securityhub' and item['detail-type'] == 'Security Hub Findings - Imported':
                ...
                key = product_name + '/' + account_id + '/' + region
            ...
            if item['source'] == 'aws.guardduty' and item['detail-type'] == 'GuardDuty Finding':
                account_id = item['detail']['accountId']
                sp = item['detail']['arn'].split('/')
                product_node = sp[0]
                product = product_node.split(':')
                product_name = product[2]
                region =  product[3]
                ...
                key = product_name + '/' + account_id + '/' + region
                findings = item['detail']
                fixed = self.fix_dictionary(findings)
                logger.info(f'fixed_finding={fixed}')
